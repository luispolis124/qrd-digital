...
async function carregarSistemaQRD() {
    try {
        ...
        const filmeAtual = catalogoQRD[cartuchoId] || {
            titulo: "Cartucho Desconhecido",
            videoUrl: "https://test-streams.mux.dev/x36xhzz/x36xhzz.m3u8" // Exemplo padrão em HLS
        };
...
function iniciarLogicaLocacao(filmeAtual) {
    ...
    if (restante > 0) {
        ...
        // Insere a tag de vídeo vazia que será controlada pelo HLS
        playerContainer.innerHTML = `
            <video id="videoPlayer" controls playsinline></video>
        `;

        const videoElement = document.getElementById('videoPlayer');
        const videoSrc = filmeAtual.videoUrl;

        // Verifica se a URL é HLS (.m3u8)
        if (videoSrc.includes('.m3u8')) {
            if (Hls.isSupported()) {
                ...
                // Suporte nativo ao HLS (Ex: Safari no iOS / Mac)
                videoElement.src = videoSrc;
            }
        } else {
            // Se for um link de vídeo comum (.mp4, .webm), roda direto
            videoElement.src = videoSrc;
        }
